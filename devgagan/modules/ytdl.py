...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    try:
        ...
        await progress_message.edit("**__📝 Editing metadata...__**")
        ...
        if os.path.exists(download_path):
            ...
            prog = await client.send_message(chat_id, "**__Starting Upload...⬆️⬆️⬆️__**")
...
async def handler(event):
    ...
    if user_id in ongoing_downloads:
        await event.reply("**You already have an ongoing download ⬇️. Please wait until it completes!**")
        return
...
async def handler(event):
    ...
    if user_id in ongoing_downloads:
        await event.reply("**You already have an ongoing ytdlp download ⬇️. Please wait until it completes!**")
        return
...
def update_progress_bar(progress, user_id):
    total_blocks = 20  # Number of blocks in the progress bar
    completed_blocks = int((progress / 100) * total_blocks)
    fractional_block = (progress / 100) * total_blocks - completed_blocks
    
    if fractional_block > 0 and completed_blocks < total_blocks:
        bar = "✅" * completed_blocks + "🟨" + "🟥" * (total_blocks - completed_blocks - 1)
    else:
        bar = "✅" * completed_blocks + "🟥" * (total_blocks - completed_blocks)
...
def progress_callback(done, total, user_id):
    ...
    update_progress_bar(percent, user_id)
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    prog = None
    progress_message = await event.reply("**__Starting download...⬇️⬇️⬇️__**")
